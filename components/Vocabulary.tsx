...
import { analyzeVocabulary, generateNewExample } from '../services/geminiService';
...
const Vocabulary: React.FC<VocabularyProps> = ({ user }) => {
  ...
  const handleAnalyze = async () => {
    ...
    for (let i = 0; i < rawInputs.length; i++) {
        const wordToAnalyze = rawInputs[i];
        setProgress(`Đang phân tích từ ${i + 1}/${rawInputs.length}: "${wordToAnalyze}"...`);
        ...
        try {
            ...
            failedWords.push(wordToAnalyze);
        }
    }
    ...
    if (failedWords.length > 0) {
        setError(`Không thể phân tích các từ: ${failedWords.join(', ')}. Vui lòng kiểm tra lại chính tả.`);
    }
  ...
  const renderLibrary = () => (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
            ...
            {isLoading && progress && <p className="text-blue-600 mt-3 text-sm flex items-center gap-2"><Loader2 className="animate-spin" size={14} /> {progress}</p>}
